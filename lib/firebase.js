...
export const getTheme = async () => {
  try {
    const s = await getDoc(doc(db, "config", "theme"));
    if (s.exists()) return { ...DEFAULT_THEME, ...s.data() };
  } catch {}
  return DEFAULT_THEME;
};
export const saveTheme = async (d) =>
  setDoc(doc(db, "config", "theme"), d, { merge: true });

// =============================================
// CATEGORIES
// =============================================
export const getCategories = async () =>
  (await getDocs(collection(db, "categories"))).docs.map((d) => ({
    id: d.id,
    ...d.data(),
  }));
...
export const calcFinalPrice = (price, discount) => {
  ...
  if (discount.type === "percent")
    return +(price * (1 - discount.value / 100)).toFixed(2);
...
export const getProducts = async (
  ...
  let products = snap.docs
    .map((d) => {
      const data = d.data();
      const { quantity, ...pub } = data;
      // Hide out-of-stock products from public (quantity === 0)
      if (quantity === 0) return null;
      return {
        id: d.id,
        ...pub,
        finalPrice: calcFinalPrice(pub.price, pub.discount),
      };
    })
    .filter(Boolean); // remove nulls (out of stock)
  ...
  if (searchQuery) {
    ...
    products = products.filter(
      (p) =>
        ...
        p.nameAr?.toLowerCase().includes(sq),
    );
  }
  return products;
};
...
export const getProductsAdmin = async () => {
  ...
  return snap.docs.map((d) => {
    ...
    return {
      id: d.id,
      ...data,
      finalPrice: calcFinalPrice(data.price, data.discount),
    };
  });
};
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onprogress = (e) => {
      if (e.lengthComputable)
        onProgress && onProgress(Math.round((e.loaded / e.total) * 50));
    };
